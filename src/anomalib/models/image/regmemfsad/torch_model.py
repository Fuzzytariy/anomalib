...
class RegMemFewShotModel(nn.Module):
    ...
    def predict(self, images: Tensor) -> AnomalyPredictions:
        ...
        raw_layer_maps: dict[str, Tensor] = {}
        ...
        for layer in self.layers:
            ...
            raw_layer_maps[layer] = anomaly_map
        ...
        if combined_map.shape[-2:] != input_size:
            combined_map = (
                F.interpolate(
                    combined_map.unsqueeze(1), size=input_size, mode="bilinear", align_corners=False
                ).squeeze(1)
            )
        layer_anomaly_maps = {
            layer: (
                map_
                if map_.shape[-2:] == input_size
                else F.interpolate(map_.unsqueeze(1), size=input_size, mode="bilinear", align_corners=False).squeeze(1)
            )
            for layer, map_ in raw_layer_maps.items()
        }
